...
class File {
	 ...
	 * @return	bool
	 */
	public static function delete_files($path, $del_dir = FALSE, $level = 0, $htdocs = FALSE)
	{
		...
		while (FALSE !== ($filename = @readdir($current_dir)))
		{
			if ($filename !== '.' && $filename !== '..')
			{
				...
				{
					static::delete_files($path.DIRECTORY_SEPARATOR.$filename, $del_dir, $level + 1, $htdocs);
				}
	...
	public static function get_filenames($source_dir, $include_path = FALSE, $_recursion = FALSE)
	{
		...
		if ($fp = @opendir($source_dir))
		{
			...
			while (FALSE !== ($file = readdir($fp)))
			{
				...
				{
					static::get_filenames($source_dir.$file.DIRECTORY_SEPARATOR, $include_path, TRUE);
				}
				elseif ($file[0] !== '.')
				{
